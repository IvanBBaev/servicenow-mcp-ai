...
- [ ] **М-1 · Премести файловете** по схемата (чист move + поправка на import-и, нула промяна в поведение). Тестовете внасят от `build/` — провери кои пътища ползват (`test/*.js` → `build/servicenow.js` и пр.) и или запази shim-ове, или обнови тестовете в същия commit. _Критерий:_ build/lint/50 теста зелени; `git diff --stat` показва основно renames.
- [ ] **М-2 · ESLint граници.** Добави `eslint-plugin-import` (или ръчно `no-restricted-imports` per директория чрез flat-config overrides): `src/core/**` не може да внася от `api|mcp|tools`; `src/api/**` не може от `mcp|tools`; `src/tools/**` не може от `core/http` директно (само през `api/`). _Критерий:_ нарочно грешен import гърми на lint.
...
- [ ] **М-3 · `mcp/define.ts`.** Типът и помощникът:
...
- [ ] **Х-7 · Email пакет** — недовършеният елемент от Фаза 2: `servicenow_send_email` (POST `/api/now/email`, destructive=false, idempotent=false), `servicenow_get_email` (GET по sys_id). Зад package `email`, извън всички профили освен `all`. С `pluginCall` обвивка (Email API изисква активиран plugin на някои инстанции).
...
- [ ] **О-1 · `sysparm_exclude_reference_link=true` по подразбиране.** Table API връща reference полета като `{ value, link }` — `link` URL-ите са чист токен-баласт за LLM. В `queryPage`/`getRecord` ([api/table.ts]) добави параметъра по подразбиране, с opt-out `SN_INCLUDE_REF_LINKS=true` за редкия случай, в който link-овете трябват. Очакван ефект: −20–40% от размера на отговор с reference полета.
- [ ] **О-2 · Компактен JSON изход.** `ok()`/`okQueryResult()` ([result.ts](src/result.ts)) ползват `JSON.stringify(_, null, 2)` — отстъпите грубо удвояват токените при големи резултати. Env `SN_RESULT_PRETTY` (default **false** → компактен изход; `true` за четим). Едно място за промяна: `asText()` + двете stringify в `okQueryResult`/`fail`. _Внимание:_ има тестове, които парсват изхода — те ползват `JSON.parse`, така че не зависят от форматирането; провери с пълния suite.
- [ ] **О-3 · Схема-кеш с TTL.** `describe_table`/`list_tables`/`get_cmdb_meta` връщат почти статични данни, а се викат често (и от resources). Малък generic `core/cache.ts`: `cached(key, ttlMs, fn)` върху `Map` с timestamp; TTL от `SN_SCHEMA_CACHE_TTL_SEC` (default 300, `0` изключва). Ключ: `host|table`. Прилага се само в `api/meta.ts` и `api/cmdb.ts#getCmdbMeta` — не генерализирай преждевременно към други четения.
- [ ] **О-4 · Лимит на паралелизма (лек).** `tableLogic` пуска 5 паралелни заявки, `fetchAll` може да направи десетки последователни — добави прост семафор в `core/http.ts` (env `SN_MAX_CONCURRENT`, default 4) около `fetch`. Пази инстанцията от салви и прави 429 по-малко вероятен. ~15 реда код, без външна зависимост.
- [ ] **О-5 · Телеметрия в процеса.** Брояч в `core/http.ts`: `{ requests, errors: {401: n, 403: n, 429: n, …}, retries, totalMs }`; експонирай го в `servicenow_get_status` и в `servicenow://status`. Нула външни зависимости; прави „защо е бавно/чупи се" диагностицируемо от самия клиент.
...
6. **Ревюто от 2026-06-12** (TODO.md, задачи `S-*`/`A-*`/`Q-*`) се изпълнява преплетено с тази фаза: S-1/S-2 (наследени колони в describe_table) — веднага, преди всичко друго; S-3/S-4/S-6/S-7 — заедно със стъпка 1 (К-задачите); Q-1/Q-3 (тестове на tools слоя + fetchAll) — преди стъпка 2 (рефакторинга); A-2 (ConfigStore) — преди Фаза 7; A-1 (per-package policy) — заедно с MI-2.
