...
- **Обмислени варианти:** (а) _read-through store_ (чете env при всяко извикване) — отхвърлен: нулева промяна спрямо сегашното, само индирекция; (б) _пълен ConfigStore за всички SN_* настройки_ (policy, settings, OAuth) — отхвърлен за сега: планът нарочно слага това след М-1/М-2 преместването в `core/`, иначе рефакторингът се прави два пъти; (в) **снапшот store само за креденшълите** — избран: малък, дава структурна атомарност и е точно опорната точка, която MI-1 ще разширява до профили.
